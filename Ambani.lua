...
local library = {
    ...
    multiZindex = 200,
    toInvis = {},
    libColor = Color3.fromRGB(112, 162, 255),
...
function library:addTab(name)
    ...
    function tab:createGroup(pos, groupname) -- newTab[pos == 0 and "left" or "right"]
                            ...
                            function(key)
                                if key.UserInputType == Enum.UserInputType.MouseButton1 then
                                    while heartbeat:wait() and
                                        ...
                                        local num =
                                            math.max(1, math.min(7, math.floor(((percent * 7 + 0.5) * 100)) / 100))
                                        ...
                                        picker.BackgroundColor3 =
                                            startC:lerp(endC, num - math.floor(num)) or Color3.new(0, 0, 0)
                                        updateValue(color)
                                    end
                                    library.colorpicking = false
                                end
                            ...
                            function(key)
                                if key.UserInputType == Enum.UserInputType.MouseButton1 then
                                    while heartbeat:wait() and
                                        ...
                                        local yPercent =
                                            (pickerY - picker.AbsolutePosition.Y - 36) / picker.AbsoluteSize.Y
                                        local color =
        ...
        function group:addButton(args)
            if not args.callback or not args.text then
                return warn("⚠️ incorrect arguments ⚠️")
            end
        ...
        function group:addSlider(args, sub)
            ...
            local function updateValue(value)
                ...
                if value ~= 0 then
                    fill:TweenSize(
                        UDim2.new(value / args.max, 0, 1, 0),
            ...
            local function updateScroll()
                if scrolling or library.scrolling or not newTab.Visible or library.colorpicking then
                    return
                end
                while inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) and menu.Enabled do
                    ...
                    local value =
                        args.min +
                        ((mouse.X - button.AbsolutePosition.X) / button.AbsoluteSize.X) * (args.max - args.min)
                    if value < 0 then
                        value = 0
                    end
                    if value > args.max then
                        value = args.max
                    end
                    if value < args.min then
                        value = args.min
                    end
        ...
        function group:addList(args)
            ...
            function refresh(tbl)
                ...
                updateValue(
                    table.find(library.options[args.flag].values, library.flags[args.flag]) and library.flags[args.flag] or
